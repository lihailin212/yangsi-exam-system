...
import { getInviteInfo, inviteLogin } from '@/api/distributions'
...
const handleLogin = async () => {
  ...
  try {
    ...
    router.push({ path: `/exam-take/${exam.value.exam_id}` })
  } catch (e) {
    loginError.value = e.response?.data?.detail || '工号或密码错误，您没有此考试的访问权限'
  } finally {
    loading.value = false
  }
}
...
onMounted(async () => {
  try {
    exam.value = await getInviteInfo(code)
  } catch {
    exam.value = null
  }
})
